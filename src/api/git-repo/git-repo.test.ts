...
test("passing relative path to isIgnored resolves relative to repo root rather than pwd", async () => {
  const result = await evaluate(
    `
      const repo = new GitRepo(GitRepo.findRoot(__dirname));
      const fixturesPath = new Path(repo.repoDir.concat("src/test_fixtures/gitignoring"));
      ...
      cd(repo.repoDir);
      cd("src/test_fixtures/gitignoring");
  ...
  expect(cleanResult(result)).toMatchInlineSnapshot(`
    {
      "code": 0,
      "error": false,
      "stderr": "exec: git check-ignore "<rootDir>/src/test_fixtures/gitignoring/hi.txt"
      exec -> {"status":1}
    exec: git check-ignore "<rootDir>/src/test_fixtures/gitignoring/subdir/hi.txt"
    ...
    exec: git check-ignore "<rootDir>/src/test_fixtures/gitignoring/subdir/hi.txt"
    ",
      "stdout": "at Path { <rootDir>/src/test_fixtures/gitignoring } repo.isIgnored("hi.txt") is: false
    at Path { <rootDir>/src/test_fixtures/gitignoring/subdir } repo.isIgnored("hi.txt") is: false
    at Path { <rootDir>/src/test_fixtures/gitignoring/subdir } repo.isIgnored(pwd().concat("hi.txt")) is: true
    ",
    }
  `);
});
  ...
  expect(cleanResult(result)).toMatchInlineSnapshot(`
    {
      "code": 1,
      "error": false,
      "stderr": "Error: The 'repoPath' provided to the GitRepo constructor doesn't appear to refer to a git repository; namely, "<rootDir>/src/api/git-repo/.git" doesn't exist. (repoDir = "<rootDir>/src/api/git-repo", dotGitDir = Path { <rootDir>/src/api/git-repo/.git })
      at somewhere
    {
      repoDir: "<rootDir>/src/api/git-repo"
      dotGitDir: Path { <rootDir>/src/api/git-repo/.git }
    }
    ",
      "stdout": "",
    }
  ...
  expect(cleanResult(result)).toMatchInlineSnapshot(`
    {
      "code": 1,
      "error": false,
      "stderr": "Error: Path passed to GitRepo.isIgnored is outside of the GitRepo object's repoDir. (path = "/tmp/something", resolvedPath = "/tmp/something", cwd = Path { <rootDir>/src/api/git-repo }, repoDir = "<rootDir>")
      at somewhere
    {
      path: "/tmp/something"
      resolvedPath: "/tmp/something"
      cwd: Path { <rootDir>/src/api/git-repo }
      repoDir: "<rootDir>"
    }
    ",
      "stdout": "",
